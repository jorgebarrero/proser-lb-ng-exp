...
router.get('/one/:id', async function(req, res, next) {
  ...
  // let buscar = req.params.myQuery;
	...
	let querySQL = `SELECT * FROM inv_campanas WHERE id_global > 0 AND id_inv_campanas = ${id}`;
...
router.post('/insert', async function(req, res, next) {
  ...
  let querySQL = `INSERT INTO inv_campanas SET ?`;
          // Data base access
          try {
...
router.put('/update/:id', async function(req, res, next) {
  ...
  let querySQL = `UPDATE inv_campanas SET ? WHERE id_inv_campanas = ` + req.params.id;
  // Data base access
  try {
    var result = await poolDat.query(querySQL, values);
    res.send(JSON.stringify(result));
...
router.delete('/delete/:id', async function(req, res, next) {
  ...
  let querySQL = `DELETE FROM inv_campanas WHERE id_inv_campanas = ` + req.params.id;
  // Data base access
  try {
    var result = await poolDat.query(querySQL);
    res.send(JSON.stringify(result));
...
router.get('/count/:myQuery?', async function(req, res, next) {
  ...
  let querySQL = `
    SELECT
      count(id_inv_campanas)  as count,
