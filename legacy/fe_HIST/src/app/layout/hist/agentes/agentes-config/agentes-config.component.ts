...
@Component({
  selector: 'app-agentes-config',
  templateUrl: './agentes-config.component.html',
  styleUrls: ['./agentes-config.component.scss']
})
export class AgentesConfigComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    let storedOptions = this.proser_store.agente.options;
    ...
    if (storedOptions) {
      this.options = this.proser_store.agente.options;
    }
  ...
  ngOnDestroy() {
    ...
    // this.onMismoDia();
    this.proser_store.agente.selection = this.selection;
    this.proser_store.agente.options = this.options;
