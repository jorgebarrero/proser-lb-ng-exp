...
// Data base access
  ...
  WHERE
  	id_inv_auxiliares <> 0  AND productividad = 0
  ${filtro_fecha_inicio}
  ${filtro_fecha_final}
  ...
  GROUP by date_text, nombre_auxiliares
...
WHERE
	id_inv_auxiliares <> 0  AND productividad = 1
${filtro_fecha_inicio}
${filtro_fecha_final}
...
try {
    ...
    .map( x => {
      return {
        cdr: x,
        audit: match( x.date_text, result_audit),
        audit_auxiliares: match_all( x.date_text, result_audit_auxiliares),
        audit_asignaciones: match_all (x.date_text, result_audit_asignaciones)
      }
    })
...
function match( date_text, audit){
  let newResult = {};
