...
function resultado_llamada(disposition) {
  // =SI.ERROR(EH4+EK4;'')
  //EH4=duracion_llamada, EK4=tiempo_after_call
...
function llamadas_atendidas(disposition, abandon_time, llamadas_recibidas) {
  // =SI(Y(DS4=1;DL4<>'');1;0)
  // DS4=llamadas_recibidas, DL4=complete

  let resultado = '';

  if (llamadas_recibidas === 1 && disposition === 'ANSWERED' && (abandon_time === null || abandon_time === undefined || abandon_time === '')) {
    return resultado = 1;
  } else {
    return resultado = 0;
  }
...
function llamadas_abandonadas(disposition, llamada_entrante, abandon_time, tiempo_llamante_abandono, llamadas_recibidas) {

  let resultado = '';

  if (abandon_time) {
    if (abandon_time.length > 0 || abandon_time !== null || abandon_time !== undefined) {
      console.log(abandon_time);
      return resultado = 1;
    } else {
      return resultado = 0;
    }
...
function llamadas_cortas(llamadas_atendidas, tiempo_agente_finalizacion, llamadas_recibidas) {
  ...
  if (llamadas_recibidas === 1 && llamadas_atendidas == 1 && tiempo_agente_finalizacion <= intervalo && tiempo_agente_finalizacion > 0) {
    return 1;
  }
...
function llamadas_antes_de_20(llamadas_atendidas, tiempo_cola_agente, llamadas_recibidas) {
  ...
  if (llamadas_recibidas === 1 && llamadas_atendidas === 1) {
