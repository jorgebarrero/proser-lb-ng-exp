...
router.get('/one/:id', async function(req, res, next) {
  ...
  let id = req.params.id;
	...
	let querySQL = `SELECT * FROM inv_colas WHERE id_global > 0 AND id_inv_colas = ${id}`;
...
router.post('/insert', async function(req, res, next) {
  ...
  let querySQL = `INSERT INTO inv_colas SET ?`;
          // Data base access
          try {
...
router.put('/update/:id', async function(req, res, next) {
  ...
  let querySQL = `UPDATE inv_colas SET ? WHERE id_inv_colas = ` + req.params.id;
  // Data base access
  try {
    var result = await poolDat.query(querySQL, values);
    res.send(JSON.stringify(result));
...
router.delete('/delete/:id', async function(req, res, next) {
  ...
  let querySQL = `DELETE FROM inv_colas WHERE id_inv_colas = ` + req.params.id;
  // Data base access
  try {
    var result = await poolDat.query(querySQL);
    res.send(JSON.stringify(result));
